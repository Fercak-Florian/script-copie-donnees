...
$cuid = Read-Host Read-Host "Veuillez saisir le CUID en majuscule"
...
Get-ChildItem "${disqueSource}:\Users\${cuid}\" -Directory -Filter ".*" -Force | ForEach-Object {
    if ($testMode) {
        robocopy "${disqueSource}:\Users\EFFI8230\$($PSItem)" "${disqueDestination}:\test-robocopy\destination\${test}\EFFI8230\$($PSItem)" /e /copyall /w:3 /r:3 /mt:16
        Read-Host "Appuyer sur un touche pour continuer"
    } else {
        robocopy "${disqueSource}:\Users\${cuid}\$($PSItem)" "${disqueDestination}:\Users\${cuid}\$($PSItem)" /e /copyall /w:3 /r:3 /mt:16
    }
}
...
foreach ($element in $data)
{
    if ($testMode) {
        ...
        robocopy $source $destination /e /copyall /w:3 /r:3 /mt:16
    } else {
        Write-Host $element
...
if ($testMode) {
    ...
    robocopy $source $destination /xd Outlook /e /copyall /w:3 /r:3 /mt:16
} else {
...
Write-Host ---- Copie des donnees Microsoft en cache dans AppData\Local ----
...
if ($testMode) {
    robocopy "${disqueSource}:\Users\EFFI8230\AppData\Local\Microsoft\OneNote" "${disqueDestination}:\test-robocopy\destination\${test}\EFFI8230\AppData\Local\Microsoft\OneNote" /e /copyall /w:3 /r:3 /mt:16
    robocopy "${disqueSource}:\Users\EFFI8230\AppData\Local\Microsoft\Outlook" "${disqueDestination}:\Users\EFFI8230\AppData\Local\Microsoft\Outlook" /e /copyall /w:3 /r:3 /mt:16
} else {
    robocopy "${disqueSource}:\Users\${cuid}\AppData\Local\Microsoft\OneNote" "${disqueDestination}:\Users\${cuid}\AppData\Local\Microsoft\OneNote" /e /copyall /w:3 /r:3 /mt:16
    robocopy "${disqueSource}:\Users\${cuid}\AppData\Local\Microsoft\Outlook" "${disqueDestination}:\Users\${cuid}\AppData\Local\Microsoft\Outlook" /e /copyall /w:3 /r:3 /mt:16
}
